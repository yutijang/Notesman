...
gchar* toFileUri(char const* path) {
    ...
    if (g_uri_parse_scheme(path) != nullptr) {
        return g_strdup(path);
    }

    // Absolute filesystem path
    if (g_path_is_absolute(path) != 0) {
...
gboolean onContextMenu(WebKitWebView* /*unused*/,
                       GtkWidget* /*unused*/,
                       WebKitHitTestResult* /*unused*/,
                       gboolean /*unused*/,
                       gpointer /*unused*/) {
...
gchar* readStdinAll() {
    ...
    while (feof(stdin) == 0) {
        ...
        if (n > 0) {
            g_string_append_len(buf, tmp, static_cast<gssize>(n));
        }
    }
