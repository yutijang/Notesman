...
class AppController final : public QObject {
  ...
  public:
    ...
    void handleAddNoteRequest(QString const& title,
  ...
  private: // NOLINT(readability-redundant-access-specifiers)
    ...
    void finalizeUnlink();
    sqlite_int64
        handleTextMode(std::string const& title, QString const& textContent, ResourceType& outType);
    sqlite_int64 handleFileMode(std::string const& title,
                                std::filesystem::path const& filePath,
                                ResourceType& outType);
    sqlite_int64 handleUrlMode(std::string const& title, QString const& url, ResourceType& outType);
