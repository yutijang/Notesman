...
class ResourceViewerFactory {
    public:
        ...
        [[nodiscard]] static std::unique_ptr<IResourceViewer>
            create(std::int64_t id, ResourceType type, QString const& title, QString const& path,
                   QString const& url, UiConst::Theme theme, ResourceViewService& viewService,
                   QWidget* parent);
};
