...
class NotesAppCore {
  public:
    ...
    [[nodiscard]]
    sqlite3_int64
        addTextNote(std::string const& title, std::string const& content, ResourceType type) const;
    [[nodiscard]]
    sqlite3_int64 addFileNote(std::filesystem::path const& filepath,
    ...
    std::optional<sqlite3_int64>
        addUrlNote(std::string_view title, ResourceType type, std::string_view rawUrl) const;
    ...
    [[nodiscard]]
    std::vector<UnifiedSearchResult> searchByContentUnifiedFull(std::string const& keyword) const;
