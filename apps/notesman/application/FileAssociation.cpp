...
namespace {
    ...
    [[nodiscard]] std::filesystem::path xdgDataHome() {
        ...
        if ((xdgEnv != nullptr) && xdgEnv[0] != '\0') { return xdgEnv; }
    ...
    [[nodiscard]] bool runCommand(QString const& program, QStringList const& args) {
        ...
        if (!ok) {
            Log::err("Command failed: {} {}, exit={}", program.toStdString(),
                     args.join(' ').toStdString(), proc.exitCode());
        }
    ...
    [[nodiscard]] bool installIcon() {
        ...
        std::filesystem::copy_file(srcIcon, dstIcon,
...
<mime-info xmlns="http://www.freedesktop.org/standards/shared-mime-info">
  <mime-type type=")"
    ...
    [[nodiscard]] bool writeDesktopFile(std::filesystem::path const& path,
    ...
    [[nodiscard]] std::string readDesktopExecPath(std::filesystem::path const& path) {
        ...
        while (std::getline(f, line)) {
            if (!line.starts_with("Exec=")) { continue; }
...
namespace FileAssociation {
    bool isUpToDate() {
        ...
        if (!std::filesystem::exists(mimePackagePath())) { return false; }
    ...
    bool registerAssociation() {
        ...
        bool runOk =
            runCommand("gtk-update-icon-cache", {"-f", "-t", QString::fromStdString(iconDir)});
        if (!runOk) { Log::err("gtk-update-icon-cache failed"); }
        ...
        if (!runOk) { Log::err("update-desktop-database failed"); }
    ...
    void unregisterAssociation() {
        ...
        if (!runOk) { Log::err("update-mime-database failed"); }
