...
class HtmlViewer final : public IResourceViewer,
  ...
  public:
    // load từ file
    static std::unique_ptr<HtmlViewer>
        createFromFile(QString title, QString path, ContentMode mode, QWidget* parent);

    // url
    static std::unique_ptr<HtmlViewer>
        createFromUrl(QString title, QUrl url, ContentMode mode, QWidget* parent);

    ~HtmlViewer() override = default;

#ifdef Q_OS_LINUX
    [[nodiscard]] bool usesExternalWindow() const override {
        return true;
    }
