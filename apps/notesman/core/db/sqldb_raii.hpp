...
class SQLiteDB {
  public:
    ...
    explicit SQLiteDB(std::string const& filename) {
        open(filename);
    }
    ...
    [[nodiscard]] std::string const& getFilename() const noexcept {
        return m_filename;
    }
...
class SQLiteStmt {
  public:
    ...
    explicit SQLiteStmt(sqlite3* db, std::string const& query) {
        if (db == nullptr) {
            throw std::invalid_argument("SQLiteStmt: db is null");
        }
    ...
    [[nodiscard]] std::string getColumnText(int index) const {
        char const* text = reinterpret_cast<char const*>(sqlite3_column_text(m_stmt.get(), index));
        if (text == nullptr) {
            return {};
        }
