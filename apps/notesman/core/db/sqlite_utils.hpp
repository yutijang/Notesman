...
inline void
    checkBind(int rc, sqlite3* db, std::source_location loc = std::source_location::current()) {
    if (rc != SQLITE_OK) {
...
inline void checkExec(sqlite3* db,
                      char const* sqlString,
                      std::string_view context,
                      std::source_location loc = std::source_location::current()) {
    char* errMsg{};
