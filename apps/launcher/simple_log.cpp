...
void write(std::wstring const& msg) {
    ...
    // Ghi Timestamp
    auto bytesToWrite =
        static_cast<DWORD>(static_cast<unsigned long long>(lstrlenW(timestamp)) * sizeof(wchar_t));
    WriteFile(h, timestamp, bytesToWrite, &written, nullptr);

    // Ghi nội dung log chính
    WriteFile(h, msg.c_str(), static_cast<DWORD>(msg.size() * sizeof(wchar_t)), &written, nullptr);
