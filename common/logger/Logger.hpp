...
void init(std::string const& loggerName = "app", std::string const& fileName = "logs/error.log");
...
struct SourceLocFmt {
    ...
    constexpr SourceLocFmt(T const& s, std::source_location l = std::source_location::current())
        : fmt(s), loc(l) {}
};
...
template<typename... Args> inline void info(SourceLocFmt sf, Args&&... args) {
    detail::get().log(spdlog::source_loc{sf.loc.file_name(),
                                         static_cast<int>(sf.loc.line()),
