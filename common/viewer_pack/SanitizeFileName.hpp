...
static bool isWindowsReservedName(std::string_view filename) {
    ...
    char upper[6] = {};
    if (base.size() >= sizeof(upper)) { return false; } // COM10 không reserved
    ...
    return std::ranges::any_of(reserved, [u](std::string_view r) { return u == r; });
}

// Kiểm tra ký tự cấm (O(1) thực sự với Lookup Table)
static constexpr std::array<bool, 256> createForbiddenLookup() {
    std::array<bool, 256> lookup = {false};
    // Các mã điều khiển ASCII (0-31)
    for (int i = 0; i < 32; ++i) { lookup[i] = true; }
    // Ký tự cấm trên Windows/Linux
    for (unsigned char c : {'<', '>', ':', '"', '/', '\\', '|', '?', '*'}) { lookup[c] = true; }
    return lookup;
}

static constexpr auto FORBIDDEN_LOOKUP = createForbiddenLookup();
...
std::string sanitizeFileName(std::string_view input, char replacement = '_') {
    ...
    if (isWindowsReservedName(result)) { result.insert(0, "_"); }

    // Giới hạn độ dài (255 bytes cho hầu hết các File System hiện đại)
    if (result.length() > 255) { result.erase(255); }
